...
type ExitResolver interface {
	Resolve(ctx context.Context, repo repository.Repository, lines []dto.ExitLineRequest, forUpdate bool) ([]ResolvedExitLine, error)
}
...
func (er *exitResolverImpl) Resolve(
	...
	batchCache := make(map[uint]*models2.InventoryBatch, len(sortedBatchIDs))
	for _, id := range sortedBatchIDs {
		var batch *models2.InventoryBatch
		var err error
		...
		if batch.IsExhausted() {
			return nil, fmt.Errorf("%w: batch %d is exhausted", Inventory.ErrInsufficientStock, batch.ID)
		}
		batchCache[id] = batch
	}
